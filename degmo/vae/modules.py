...
class ConvEncoder(torch.nn.Module):
    def __init__(self, c, h, w, latent_dim, features, down_sampling, res_layers):
        super().__init__()

        feature_shape = (features, h // (2 ** down_sampling), w // (2 ** down_sampling))
        
        conv_features = features // (2 ** down_sampling)
        encoder_list = [
        ...
        encoder_list.append(torch.nn.Linear(np.prod(feature_shape), 2 * latent_dim))
...
class ConvDecoder(torch.nn.Module):
    def __init__(self, output_c, h, w, latent_dim, features, down_sampling, res_layers):
        super().__init__()
        
        feature_shape = (features, h // (2 ** down_sampling), w // (2 ** down_sampling))

        decoder_list = [
            torch.nn.Linear(latent_dim, np.prod(feature_shape)),
            Unflatten(*feature_shape),
            ResNet(features, features, features, res_layers),
        ]
